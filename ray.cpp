...
Vec3f Ray::cast(const std::vector<Sphere> &spheres, const std::vector<Light> &lights)
{
    ...
    for (size_t i=0; i<lights.size(); i++) {
        ...
        diffuse_light_intensity  += lights[i].intensity * std::max(0.0f, light_dir.dot(N));
        specular_light_intensity += powf(std::max(0.0f, Vec3f::reflect(light_dir, N).dot(m_direction)), material.spec_exp)*lights[i].intensity;
